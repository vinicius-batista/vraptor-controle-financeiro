...
public class DAOFactory {
  ...
  public ReceitaDAO criarReceitaDAO() {
    if (conexao == null) {
      ...
      return new ReceitaDAO(conexao);
    }
  }

  public DespesaDAO criarDespesaDAO() {
    if (conexao == null) {
      throw new IllegalStateException("Abra uma conexão antes de criar um DAO.");
    } else {
      return new DespesaDAO(conexao);
    }
  }

  public ControleMensalDAO criarControleMensalDAO() {
    if (conexao == null) {
      throw new IllegalStateException("Abra uma conexão antes de criar um DAO.");
    } else {
      return new ControleMensalDAO(conexao);
    }
  }
